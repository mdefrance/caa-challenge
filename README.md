...
> 📝 **An article on this work is in review.** It walks through the method and revisits it
> with the current library; a link goes here once it is published. Every figure it quotes
> was measured on the notebooks in this repository, on one machine, with nothing else
> running, and every measurement has a script here that regenerates it.
...
kaggle/                              # Kaggle kernel metadata; code generated by tools/build_kaggle.py
...
- [severity model](https://www.kaggle.com/code/mariodefrance/caa-amount-model) — reads the
  article's own frequency hand-off from the dataset, so it runs on its own
- [dataset](https://www.kaggle.com/datasets/mariodefrance/caa-challenge-2025)

The Kaggle copies are **generated** from `src/` by `tools/build_kaggle.py` (see
[`kaggle/README.md`](kaggle/README.md)) and run a reduced budget (30 Optuna trials,
`N_JOBS = 1`) to fit a free CPU session, so their numbers do not match the article's. The
notebooks in `src/` are the ones the article quotes.
